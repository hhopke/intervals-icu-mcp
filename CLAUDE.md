...
- **Remove the synthesized zone blocks from the curve tools.** `icu_get_hr_curves` returns an `hr_zones` block and `icu_get_power_curves` an `ftp_analysis.power_zones` block, both built from hardcoded percentage tables rather than the athlete's configured zones — a second, worse answer to what `icu_get_sport_settings` now returns correctly, under colliding key names. The HR one is anchored to the curve peak, which Intervals.icu has already clipped to the configured max HR at import, so it can only echo a stale value of the setting it substitutes for. Non-breaking interim (shipped): `summary.max_hr_bpm` renamed to `peak_hr_bpm`, and both blocks carry a `DEPRECATED` comment (`tools/curves.py`, `tools/performance.py`). Breaking cleanup to batch: delete both blocks, keeping `estimated_ftp` and `twenty_min_power`, which are genuine measurements. See #119.
